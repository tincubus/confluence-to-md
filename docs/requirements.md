...
- **Folder per Page, Note is `{name}.md` inside that folder.** Hierarchy is visible on disk, and a move of the folder keeps the Note and its Attachments together. `{name}` is the sanitized Page title, not `index.md`: Obsidian names a note from the filename, with no plugin, so every note would otherwise appear as `index`.
- **Vault folder names drop emoji; the Site title stays in frontmatter.** Emoji in a Space or Page name make paths and Obsidian filenames noisy. The Space folder, the Page folder, the Note filename, and Vault Page links use the sanitized name so those links still resolve. Frontmatter `title` and `list` keep the Confluence name so the Operator can still match the Site. Attachment filenames and Note bodies are not stripped.
...
Names in this tree are sanitized folder names, not the raw Confluence titles.

- Every Page, including the homepage, is a folder. The Note is `{name}.md` inside that folder, where `{name}` is the sanitized Page title. Children nest inside the parent folder. The Space folder itself is not a Note. The Space folder name is the sanitized Space name.
...
- Sanitize a Space folder, a Page folder, and the Note filename in this order: remove Unicode emoji and pictographs (including ZWJ sequences and emoji presentation variants); trim and collapse whitespace; replace `\ / : * ? " < > |` with `-`; then keep the name case-insensitively unique among siblings. A collision or a name empty after sanitizing appends ` (Page id)` for a Page folder or ` (Space key)` for a Space folder. Example: `🔬 Competitor Research (ข้อมูลคู่แข่ง/Market)` becomes `Competitor Research (ข้อมูลคู่แข่ง-Market)`.
- Frontmatter `title` is the Page title as stored on the Site, including emoji. The `list` command prints Space names as stored on the Site.
- Note body and Attachments are UTF-8 files. Filenames stay human-readable. Do not strip emoji from Note bodies or Attachment filenames.
...
- `title` is the Page title as stored on the Site, including emoji.
- `labels` copies Labels. Omit the key when there are none.
- `updated_at` is the Page's last-updated time in UTC, ISO-8601.
- The body starts with a one-line source link to `source_url`, using the Site title as the link text, then the converted body.
...
- `Child pages` lists every child Page that is in the Selection, in Confluence sibling order, then by title. Link text is the sanitized name. The href is that child's `{name}/{name}.md`. The section is omitted when there are no such children.
...
Rewrite every Confluence link whose target Page is in the Selection to a relative markdown link from the linking Note's directory to that Note's sanitized `{name}.md`. Link text is the sanitized name. Encode spaces and reserved characters. Use `../` to climb. No wikilink syntax.
...
- Sanitize attachment filenames: trim, replace `\ / : * ? " < > |` with `-`. Do not strip emoji. A name collision in one `attachments/` directory gets the attachment id appended.
...
- Title, parent, or sanitized folder-name change moves the folder to the new Hierarchy path, then writes the new Note. Page links are regenerated from Confluence, so they follow the move.
...
1. A Space with a homepage, two children, and one grandchild exports to the nested layout above. The homepage Note is `{Space}/{Homepage name}/{Homepage name}.md`, using sanitized folder names. Each `Child pages` section links to the children that were exported, with sanitized link text and href. Obsidian's file explorer shows those names, not `index`.
2. Page A links to Page B by title, and both are in the Selection. A's Note contains a relative markdown link to B's sanitized `{name}.md`, with sanitized link text. Following it opens B with no network.
...
14. A Space or Page whose name contains emoji exports to a folder and Note named without those emoji. Frontmatter `title` still has them. A Page link to that Page uses the sanitized path and sanitized link text, and opens the Note with no network. `list` still prints the Space name as stored on the Site.
